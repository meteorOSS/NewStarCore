...
public class Kettle extends Icmd {

    public Kettle(StarCore plugin) {
        super(plugin);
    }
        ...
        switch (i){
            ...
            case 2:
                if(args[1].equalsIgnoreCase("get")){
                    return new ArrayList<>(Config.getInstance().getKettleMap().keySet());
    ...
    public void perform(CommandSender p0, String[] p1) {
        com.meteor.starcore.data.plant.Kettle kettle = null;
        ItemStack itemStack = null;
        try {
            switch (p1[1]){
                case "get":
                    kettle = Config.getInstance().getKettleMap().get(p1[2]);
                    itemStack = kettle.toItemStack();
                    itemStack.setAmount(Integer.valueOf(p1[3]));
                    ((Player)p0).getInventory().addItem(itemStack);
                    p0.sendMessage(Config.getInstance().getMessageManager().getString("message.get").replace("@item@",kettle.getName()).replace("@amount@",p1[3]));
                    return;
                case "give":
                    ...
                    kettle = Config.getInstance().getKettleMap().get(p1[3]);
                    itemStack = kettle.toItemStack();
                    itemStack.setAmount(Integer.valueOf(p1[4]));
                    p.getInventory().addItem(itemStack);
                    p0.sendMessage(Config.getInstance().getMessageManager().getString("message.give").replace("@item@",kettle.getName()).replace("@amount@",p1[4])
                            .replace("@p@",p.getName()));
                    return;
                case "list":
                    StringBuilder stringBuilder = new StringBuilder();
                    Config.getInstance().getKettleMap().forEach((k,v)->{
